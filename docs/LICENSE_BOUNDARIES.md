...
WebUtau includes `WebUtau Korean V3 Synthetic`, a generated UTAU-style Korean starter voicebank. Its packaged WAV samples are generated by WebUtau DSP tooling, not copied from a human singer, TTS service output, model checkpoint output, Kasane Teto, Vocaloid, or OpenUtau asset. Keep its generation script, manifest, and bundled license text in sync when changing the asset.
