...
- Unit coverage re-imports the DAW bundle's native `.webutau.json`, USTX, classic UST, and exported melody MIDI data, then verifies the starter lyric, pitch, timing, BPM, and chord guide survive the handoff. The MIDI round-trip coverage runs across all twelve varied starter samples, not just the default hook, preserves simple pitch bends through pitch wheel events, and multitrack plus type-0 MIDI coverage verifies chord-guide notes are not imported as vocal notes.
