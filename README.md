...
The first screen is intentionally action-led: the starter card shows `START HERE`, a three-step `01 샘플 듣기` / `02 가사 적용` / `03 WAV 받기` checklist, a focused `STEP 01` / `샘플 먼저 듣기` launch panel, an inline starter lyric input, the current lyric preview, and a visible `작업 시작` utility row for `멜로디 추천`, `새 프로젝트`, and `기본 샘플` before the deeper editor controls.
...
- First-run starter panel with a clear `START HERE` / `01 샘플 듣기` / `02 가사 적용` / `03 WAV 받기` checklist, focused `STEP 01` launcher, inline lyric input, current lyric preview, visible `작업 시작` utilities, compose, play, and WAV handoff actions.
...
2. Confirm the starter card says `START HERE`, `01 샘플 듣기`, `02 가사 적용`, `03 WAV 받기`, `STEP 01`, `샘플 먼저 듣기`, and `작업 시작`.
...
- The first-run browser smoke verifies the `START HERE` / `01 샘플 듣기` / `02 가사 적용` / `03 WAV 받기` checklist, `STEP 01` / `샘플 먼저 듣기` focused launch panel, inline starter lyric input, `작업 시작` utilities, `멜로디 추천`, `새 프로젝트`, `기본 샘플`, and current lyric preview are visible before WAV handoff.
