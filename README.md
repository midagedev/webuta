...
The first screen is intentionally action-led: the starter card shows `VOCAL STARTER`, a three-step `01 샘플 듣기` / `02 가사 적용` / `03 WAV 받기` checklist, `다음 행동`, the focused `샘플 먼저 듣기` next action with a current-step badge, an inline starter lyric input, `처음 1분`, large starter cards for `샘플 듣기`, `가사 바꾸기`, `멜로디 추천`, and `WAV 받기`, the current lyric preview, and a folded `프로젝트 도구` row before the deeper editor controls.
...
- First-run starter panel with a clear `VOCAL STARTER` / `01 샘플 듣기` / `02 가사 적용` / `03 WAV 받기` checklist, `처음 1분` / `다음 행동` launcher, inline lyric input, current lyric preview, folded project tools, compose, play, and WAV handoff actions.
...
2. Confirm the starter card says `VOCAL STARTER`, `01 샘플 듣기`, `02 가사 적용`, `03 WAV 받기`, `다음 행동`, and `샘플 듣기 → 가사 바꾸기 → WAV 받기`.
...
- The first-run browser smoke verifies the `VOCAL STARTER` / `01 샘플 듣기` / `02 가사 적용` / `03 WAV 받기` checklist, `다음 행동` / `샘플 먼저 듣기` focused next action, current-step badge, inline starter lyric input, `처음 1분` route, `샘플 듣기`, `가사 바꾸기`, `멜로디 추천`, `WAV 받기`, folded `프로젝트 도구`, and current lyric preview are visible before WAV handoff.
