...
The first screen is intentionally action-led: the starter card shows `QUICK START`, `처음이면`, the focused `첫 샘플 듣기` next action with a current-step badge, an inline starter lyric input, `처음 1분`, beginner actions for `샘플 듣기 / 가사·멜로디 / WAV 받기`, the current lyric preview, the `01 보이스 확인` / `02 먼저 들어보기` / `03 WAV 저장` path, and the `미리듣기` / `가사·음정` / `WAV 저장` sketch cues before the deeper editor controls.
...
- First-run starter panel with a clear `QUICK START` / `처음 1분` / `처음이면` CTA, inline lyric input, current lyric preview, new-project, demo restore, compose, play, and WAV handoff actions.
...
2. Confirm the starter card says `QUICK START`, `처음이면`, `첫 샘플 듣기`, and `먼저 들어보기`.
...
- The first-run browser smoke verifies the `QUICK START` / `처음이면` / `첫 샘플 듣기` focused next action, current-step badge, inline starter lyric input, `처음 1분` mission, `샘플 듣기 / 가사·멜로디 / WAV 받기`, `먼저 들어보기`, `새 프로젝트`, current lyric preview, `미리듣기` / `가사·음정` / `WAV 저장` sketch cues, and `01 보이스 확인` / `02 먼저 들어보기` / `03 WAV 저장` guided path are visible before WAV handoff.
