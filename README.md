...
- OpenUtau USTX `pitch.data` curves are preserved on import/export and rendered as WebUtau pitch bends with millisecond point timing and basic curve shapes.
...
2. Keep `WebUtau Korean V3 Synthetic` selected, or import a local UTAU zip with `ZIP`.
3. Edit the lyric line. The default is `도히도히 다이스키`.
4. Press `적용` to assign lyrics to the notes.
5. Use `SEL` to loop the selected note or phrase while editing.
6. Press play to audition.
7. Press `WAV` or `공유`.
8. Use the rendered WAV in your music project.
...
- Unit tests verify UST `Intensity`, `Velocity`/`Modulation`/`Flags`, timing overrides, `Envelope`, classic pitch-bend curves, and OpenUtau USTX `pitch.data` round-trip where supported.
