...
The first screen is intentionally action-led: the starter card shows `처음 시작`, `듣기 · 가사 · WAV`, a visible `0/3` progress badge, and a `1분 미션` board that frames the goal as `한글 한 줄을 보컬 WAV로 만들기` with `First-Vocal-Sketch.wav` as the starter output. A dominant `처음이면 여기부터` panel keeps `초보자 첫 버튼`, `첫 사용 순서`, `지금 할 일`, and a top `빠른 가사 입력` / `빠른 가사 적용` lane together, so listening and lyric entry happen in the same beginner card. It also keeps a `한국어 UTAU 모드` strip for Hangul alias matching, a clear default/draft project decision, a compact `처음 1분 가이드`, a `C -> G -> Am -> F` starter chord guide, and a three-step `01 샘플 듣기` / `02 가사 바꾸기` / `03 WAV 받기` route with `지금` / `다음` / `완료` / `준비` state badges. Project details are tucked behind `현재 프로젝트`, detailed lyric examples sit behind `가사 자세히`, and optional `멜로디 추천`, `DAW 번들`, `새 프로젝트`, and `기본 샘플` actions sit behind `추가 작업` on desktop and mobile.
...
- First-run starter panel with a clear `처음 시작` / `듣기 · 가사 · WAV` header, `1분 미션` success board for `한글 한 줄을 보컬 WAV로 만들기`, `First-Vocal-Sketch.wav` as the visible first output, `처음이면 여기부터` beginner start panel, `초보자 첫 버튼`, `첫 사용 순서`, `지금 할 일`, top `빠른 가사 입력` / `빠른 가사 적용`, `한국어 UTAU 모드` alias/WAV path, obvious saved-draft/default-sample decision, `처음 1분 가이드`, `C -> G -> Am -> F` starter chord guide, `01 샘플 듣기` / `02 가사 바꾸기` / `03 WAV 받기` route with visible step-state badges, detailed lyric input behind `가사 자세히`, current lyric preview, collapsed `현재 프로젝트` and `추가 작업` sections, optional desktop/mobile compose/WAV/DAW/project actions, and a collapsed `고급 도구` area for the `다운로드 패키지` review links.
- Piano-roll note editing with draw, drag, resize, duplicate, split, delete, keyboard movement, undo, redo, and an arrangement chord lane that keeps the starter `C -> G -> Am -> F` guide aligned to the vocal timeline.
...
- Public V3 listening review scorecard under `review/v3/`, linked from the app, for scoring generated V3 WAVs against the legacy V2 baseline with a `10-minute listening review path`.
...
2. Confirm the starter card says `처음 시작`, `듣기 · 가사 · WAV`, `0/3`, `1분 미션`, `한글 한 줄을 보컬 WAV로 만들기`, `First-Vocal-Sketch.wav`, `처음이면 여기부터`, `초보자 첫 버튼`, `첫 사용 순서`, `지금 할 일`, `빠른 가사 입력`, `빠른 가사 적용`, `한국어 UTAU 모드`, `처음 1분 가이드`, `01 샘플 듣기`, `02 가사 바꾸기`, `03 WAV 받기`, `한글 그대로 입력`, `추가 작업`, `DAW 번들`, `현재 프로젝트`, and `고급 도구`.
...
- The first-run browser smoke verifies the `처음 시작` / `듣기 · 가사 · WAV` header, `1분 미션` success board, `한글 한 줄을 보컬 WAV로 만들기`, visible `First-Vocal-Sketch.wav` output, `처음이면 여기부터` beginner start panel, `초보자 첫 버튼`, `첫 사용 순서`, `지금 할 일`, top `빠른 가사 입력` / `빠른 가사 적용`, `한국어 UTAU 모드` alias/WAV path, collapsed `현재 프로젝트` context drawer, `처음 1분 가이드`, `C -> G -> Am -> F` chord guide, `01 샘플 듣기` / `02 가사 바꾸기` / `03 WAV 받기` route with `지금` / `다음` state badges, detailed starter lyric input with `한글 그대로 입력` behind `가사 자세히`, collapsed `추가 작업` utilities for `멜로디 추천`, `DAW 번들`, `새 프로젝트`, `기본 샘플`, and current lyric preview are reachable before WAV/DAW handoff.
...
- The public review hub includes an `Evidence Preflight` checker that validates the two downloaded JSON files locally in the browser with `No upload`, before reviewers run `npm run release:evidence-status`.
