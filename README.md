...
- Public release review hub under `review/`, linked from the app, that routes reviewers to the two final evidence builders and repeats the `2 files to finish` acceptance path.
...
- The public review hub includes an `Evidence Preflight` checker that validates the two downloaded JSON files locally in the browser with `No upload`, including `realPlaybackConfirmed`, `lyricBlindPassConfirmed`, and `v2ComparisonConfirmed` in the listening JSON before reviewers run `npm run release:evidence-status`.
