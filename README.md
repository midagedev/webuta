...
The first screen is intentionally action-led: the starter card shows `지금 할 일`, a large `먼저 들어보기` / `WAV 받기` button, the default lyric preview, and the `01 보이스` / `02 재생` / `03 WAV` path before the deeper editor controls.
...
- First-run starter panel with a clear `지금 할 일` CTA, default lyric preview, `새 프로젝트`, demo restore, compose, play, and WAV handoff actions.
...
2. Confirm the starter card says `지금 할 일` and `먼저 들어보기`.
...
- The first-run browser smoke verifies the `지금 할 일` CTA, `먼저 들어보기`, `새 프로젝트`, and `01 보이스` / `02 재생` / `03 WAV` guided path are visible before WAV handoff.
