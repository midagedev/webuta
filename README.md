...
The first screen is intentionally action-led: the starter card shows `처음 시작`, `듣기 · 가사 · WAV`, a visible `0/3` progress badge, a dominant `처음이면 여기부터` panel with `초보자 첫 버튼`, a clear default/draft project decision, a compact `1분 완성 루트`, a `C -> G -> Am -> F` starter chord guide, and a three-step `01 샘플 듣기` / `02 가사 적용` / `03 WAV 받기` route with `지금` / `다음` / `완료` / `준비` state badges. Project details are tucked behind `현재 프로젝트`, while lyric editing, `멜로디 추천`, `DAW 번들`, `새 프로젝트`, and `기본 샘플` stay visible on desktop and mobile.
...
- First-run starter panel with a clear `처음 시작` / `듣기 · 가사 · WAV` header, `처음이면 여기부터` beginner start panel, `초보자 첫 버튼`, obvious saved-draft/default-sample decision, `1분 완성 루트`, `C -> G -> Am -> F` starter chord guide, `01 샘플 듣기` / `02 가사 적용` / `03 WAV 받기` route with visible step-state badges, inline lyric input with Hangul examples, current lyric preview, desktop/mobile compose/play/WAV/DAW bundle actions, a collapsed `현재 프로젝트` context drawer, and a collapsed `고급 도구` area for the `다운로드 패키지` review links.
...
2. Confirm the starter card says `처음 시작`, `듣기 · 가사 · WAV`, `0/3`, `처음이면 여기부터`, `초보자 첫 버튼`, `1분 완성 루트`, `01 샘플 듣기`, `02 가사 적용`, `03 WAV 받기`, `한글 그대로 입력`, `DAW 번들`, `현재 프로젝트`, and `고급 도구`.
