...
The first screen is intentionally action-led: the starter card shows `START HERE`, a `처음 3분` route map for `듣기` / `가사` / `저장`, a three-step `01 샘플 듣기` / `02 가사 적용` / `03 WAV 받기` checklist, a focused `지금 할 일 · STEP 01` / `샘플 먼저 듣기` launch panel, an inline starter lyric input with `한글 그대로 입력` examples, the current lyric preview, a visible `다음 선택` utility row for `멜로디 추천`, `DAW 번들`, `새 프로젝트`, and `기본 샘플`, plus a `다운로드 패키지` handoff strip for `WAV · lyrics.txt · notes.csv` and review links before the deeper editor controls.
...
- First-run starter panel with a clear `START HERE` / `처음 3분` route map, `01 샘플 듣기` / `02 가사 적용` / `03 WAV 받기` checklist, focused `지금 할 일 · STEP 01` launcher, inline lyric input with Hangul examples, current lyric preview, visible `다음 선택` utilities, compose, play, WAV, DAW bundle handoff actions, and a compact `다운로드 패키지` strip for ZIP contents plus review links.
...
2. Confirm the starter card says `START HERE`, `처음 3분`, `듣기`, `가사`, `저장`, `01 샘플 듣기`, `02 가사 적용`, `03 WAV 받기`, `지금 할 일 · STEP 01`, `샘플 먼저 듣기`, `한글 그대로 입력`, `다음 선택`, `DAW 번들`, and `다운로드 패키지`.
...
- The first-run browser smoke verifies the `START HERE` / `처음 3분` route map, `01 샘플 듣기` / `02 가사 적용` / `03 WAV 받기` checklist, `지금 할 일 · STEP 01` / `샘플 먼저 듣기` focused launch panel, inline starter lyric input with `한글 그대로 입력`, `다음 선택` utilities, `멜로디 추천`, `DAW 번들`, `새 프로젝트`, `기본 샘플`, and current lyric preview are visible before WAV/DAW handoff.
