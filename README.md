...
- Native `.webutau.json` project save/import, Standard MIDI melody import, plus classic UST and USTX import/export for UTAU/OpenUtau handoff.
...
- Unit coverage re-imports the DAW bundle's native `.webutau.json`, USTX, classic UST, and exported melody MIDI data, then verifies the starter lyric, pitch, timing, BPM, and chord guide survive the handoff. The MIDI round-trip coverage runs across all twelve varied starter samples, not just the default hook.
