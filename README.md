...
The first screen is intentionally action-led: the starter card shows `QUICK START`, `처음이면`, the focused `첫 샘플 듣기` next action with a current-step badge, an inline starter lyric input, `처음 1분`, large beginner cards for `샘플 듣기`, `가사 적용`, `멜로디 만들기`, and `WAV 받기`, the current lyric preview, and the `미리듣기` / `가사·음정` / `WAV 저장` sketch cues before the deeper editor controls.
...
2. Confirm the starter card says `QUICK START`, `처음이면`, and `샘플 듣기 → 가사 적용 → 멜로디 만들기 → WAV 받기`.
...
5. Press `가사 적용` or `적용` to assign lyrics to the notes.
6. Press `멜로디 만들기` for the compose panel, or edit notes directly in the piano roll.
...
- The first-run browser smoke verifies the `QUICK START` / `처음이면` / `첫 샘플 듣기` focused next action, current-step badge, inline starter lyric input, `처음 1분` route, `샘플 듣기`, `가사 적용`, `멜로디 만들기`, `WAV 받기`, `새 프로젝트`, current lyric preview, and `미리듣기` / `가사·음정` / `WAV 저장` sketch cues are visible before WAV handoff.
