...
The first screen is intentionally action-led: the starter card shows `처음 시작`, `듣기 · 가사 · WAV`, a visible `0/3` progress badge, a dominant `처음이면 여기부터` panel with `초보자 첫 버튼`, `첫 사용 순서`, a clear default/draft project decision, a compact `첫 완성 루트`, a `C -> G -> Am -> F` starter chord guide, and a three-step `01 샘플 듣기` / `02 가사 적용` / `03 WAV 받기` route with `지금` / `다음` / `완료` / `준비` state badges. Project details are tucked behind `현재 프로젝트`, while lyric editing stays visible and optional `멜로디 추천`, `DAW 번들`, `새 프로젝트`, and `기본 샘플` actions sit behind `추가 작업` on desktop and mobile.
...
2. Confirm the starter card says `처음 시작`, `듣기 · 가사 · WAV`, `0/3`, `처음이면 여기부터`, `초보자 첫 버튼`, `첫 사용 순서`, `첫 완성 루트`, `01 샘플 듣기`, `02 가사 적용`, `03 WAV 받기`, `한글 그대로 입력`, `추가 작업`, `DAW 번들`, `현재 프로젝트`, and `고급 도구`.
...
- The first-run browser smoke verifies the `처음 시작` / `듣기 · 가사 · WAV` header, `처음이면 여기부터` beginner start panel, `초보자 첫 버튼`, `첫 사용 순서`, collapsed `현재 프로젝트` context drawer, `첫 완성 루트`, `C -> G -> Am -> F` chord guide, `01 샘플 듣기` / `02 가사 적용` / `03 WAV 받기` route with `지금` / `다음` state badges, inline starter lyric input with `한글 그대로 입력`, collapsed `추가 작업` utilities for `멜로디 추천`, `DAW 번들`, `새 프로젝트`, `기본 샘플`, and current lyric preview are visible before WAV/DAW handoff.
