...
The first screen is intentionally action-led: the starter card shows `처음 시작`, `듣기 · 가사 · WAV`, a visible `0/3` progress badge, and a `1분 미션` board that frames the goal as `한글 한 줄을 보컬 WAV로 만들기` with `First-Vocal-Sketch.wav` as the starter output. A dominant `처음이면 여기부터` panel keeps `초보자 첫 버튼`, `첫 사용 순서`, `지금 할 일`, and a top `빠른 가사 입력` / `빠른 가사 적용` lane together, so listening and lyric entry happen in the same beginner card. It also keeps a `샘플 고르기` gallery with `보컬로이드풍 훅 10개` (`Neon Lift`, `Blue Hour`, `Retro Run`, `Moon Signal`, `Pink Noise`, `Rain Verse`, `City Glide`, `Glass Pulse`, `Lofi Diary`, `Zero Gravity`) across different genres, tempos, lyric moods, note ranges, and Hangul coda patterns, with a `선택 중` sample guide plus `처음 듣는 기본 후렴`, `밝은 상승 멜로디`, `긴 끝음`, and `BPM/음역/노트/받침/끝음` comparison chips on each card. A `한국어 UTAU 모드` strip handles Hangul alias matching, followed by a clear default/draft project decision, a compact `처음 1분 가이드`, a `Am -> F -> C -> G` starter chord guide, and a three-step `01 샘플 듣기` / `02 가사 바꾸기` / `03 WAV 받기` route with `지금` / `다음` / `완료` / `준비` state badges. Project details are tucked behind `현재 프로젝트`, detailed lyric examples sit behind `가사 자세히`, and optional `멜로디 추천`, `DAW 번들`, `새 프로젝트`, and `기본 샘플` actions sit behind `추가 작업` on desktop and mobile.
