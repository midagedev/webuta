...
The first screen is intentionally action-led: the starter card shows `QUICK START`, `처음 1분`, `처음이면`, a large `먼저 들어보기` / `WAV 받기` button, beginner actions for `샘플 듣기 / 가사·멜로디 / WAV 받기`, the current lyric preview, the `01 보이스 확인` / `02 먼저 들어보기` / `03 WAV 저장` path, and the `미리듣기` / `가사·음정` / `WAV 저장` sketch cues before the deeper editor controls.
...
- First-run starter panel with a clear `QUICK START` / `처음 1분` / `처음이면` CTA, current lyric preview, new-project, demo restore, compose, play, and WAV handoff actions.
...
- UST/USTX tempo map preservation, with rendered note timing calculated from tempo events instead of only one global BPM.
...
2. Confirm the starter card says `QUICK START`, `처음 1분`, `처음이면`, and `먼저 들어보기`.
...
5. Press `적용` to assign lyrics to the notes.
6. Use `SEL` to loop the selected note or phrase while editing.
7. Press play to audition.
8. Press `WAV`, `WAV 받기`, or `공유`.
...
- The first-run browser smoke verifies the `QUICK START` / `처음 1분` / `처음이면` CTA, `샘플 듣기 / 가사·멜로디 / WAV 받기`, `먼저 들어보기`, `새 프로젝트`, current lyric preview, `미리듣기` / `가사·음정` / `WAV 저장` sketch cues, and `01 보이스 확인` / `02 먼저 들어보기` / `03 WAV 저장` guided path are visible before WAV handoff.
