...
- UTAU import compatibility smoke coverage for diverse fixture banks: Japanese CV, VCV, `prefix.map` multipitch, Shift-JIS oto.ini, legacy `character.txt` metadata, Hangul CV/VC coda, and multi-oto style ranking all render through the browser sample path.
...
- `npm run voicebank:compatibility-utau` builds diverse local fixture UTAU zips and verifies Japanese CV, Japanese VCV, `prefix.map` multipitch, Shift-JIS oto.ini, legacy `character.txt` metadata, Hangul CV/VC coda, and multi-oto style ranking render non-silent 44.1 kHz audio through the browser UTAU sample renderer without fallback aliases or render warnings.
