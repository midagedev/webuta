...
The first screen is intentionally action-led: the starter card shows `START HERE`, a three-step `01 샘플 듣기` / `02 가사 적용` / `03 WAV 받기` checklist, a focused `지금 할 일 · STEP 01` / `샘플 먼저 듣기` launch panel, an inline starter lyric input, the current lyric preview, and a visible `다음 선택` utility row for `멜로디 추천`, `DAW 번들`, `새 프로젝트`, and `기본 샘플` before the deeper editor controls.
...
- First-run starter panel with a clear `START HERE` / `01 샘플 듣기` / `02 가사 적용` / `03 WAV 받기` checklist, focused `지금 할 일 · STEP 01` launcher, inline lyric input, current lyric preview, visible `다음 선택` utilities, compose, play, WAV, and DAW bundle handoff actions.
...
2. Confirm the starter card says `START HERE`, `01 샘플 듣기`, `02 가사 적용`, `03 WAV 받기`, `지금 할 일 · STEP 01`, `샘플 먼저 듣기`, `다음 선택`, and `DAW 번들`.
...
9. Use the rendered WAV or DAW bundle in your music project.
