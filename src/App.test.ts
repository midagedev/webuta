...
describe('App editing workflow', () => {
  ...
  it('surfaces a first-run guide with direct starter actions', async () => {
    ...
    expect(within(guide).getByText('VOCAL STARTER')).toBeTruthy()
    ...
    expect(within(guide).getByText('샘플 듣기 → 가사 바꾸기 → WAV 받기')).toBeTruthy()
    expect(within(guide).getByLabelText('Starter action route')).toBeTruthy()
    expect(within(guide).getByLabelText('Starter project utilities')).toBeTruthy()
    ...
    const quickChecklist = within(guide).getByLabelText('Starter quick checklist')
    expect(within(quickChecklist).getByRole('button', { name: '첫 단계 샘플 듣기' })).toBeTruthy()
    expect(within(quickChecklist).getByRole('button', { name: '둘째 단계 가사 적용' })).toBeTruthy()
    expect(within(quickChecklist).getByRole('button', { name: '셋째 단계 WAV 받기' })).toBeTruthy()
    expect(within(quickChecklist).getByText('01')).toBeTruthy()
    expect(within(quickChecklist).getByText('02')).toBeTruthy()
    ...
    expect(within(guide).getByText('다음 행동')).toBeTruthy()
    expect(within(guide).getByLabelText('Starter next action').textContent).toContain('다음 행동')
    expect(within(guide).getByLabelText('Current starter step')).toBeTruthy()
    expect(within(guide).getByText('현재 가사')).toBeTruthy()
    expect(within(guide).getByText('기본 샘플')).toBeTruthy()
    ...
    expect(within(path).getByText('1')).toBeTruthy()
    expect(within(path).getByText('샘플 듣기')).toBeTruthy()
    expect(within(path).getByText('2')).toBeTruthy()
    expect(within(path).getByText('가사 바꾸기')).toBeTruthy()
    expect(within(path).getByText('3')).toBeTruthy()
    expect(within(path).getByText('멜로디 추천')).toBeTruthy()
    expect(within(path).getByText('4')).toBeTruthy()
    expect(within(path).getByText('WAV 받기')).toBeTruthy()
    ...
    expect(within(guide).getByRole('button', { name: '스타터 샘플 듣기' })).toBeTruthy()
    expect(within(guide).getByRole('button', { name: '스타터 가사 바꾸기' })).toBeTruthy()
    expect(within(guide).getByRole('button', { name: '스타터 멜로디 추천' })).toBeTruthy()
    expect(within(guide).getByRole('button', { name: '스타터 WAV 받기' })).toBeTruthy()
    ...
    fireEvent.click(within(guide).getByRole('button', { name: '스타터 멜로디 추천' }))
