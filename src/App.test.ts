...
describe('App editing workflow', () => {
  ...
  it('surfaces a first-run guide with direct starter actions', async () => {
    ...
    const utilities = within(guide).getByLabelText('Starter project utilities')
    ...
    expect(oneMinutePath.textContent).toContain('1분 완성 루트')
    expect(oneMinutePath.textContent).toContain('샘플 듣기 -> 가사 바꾸기 -> WAV 저장')
    expect(within(oneMinutePath).getByLabelText('Starter readiness snapshot')).toBeTruthy()
    expect(launchPanel.textContent).toContain('STEP 01')
    expect(launchPanel.textContent).toContain('지금 누를 버튼')
    ...
    expect(routeSummary.textContent).toContain('가사 적용')
    ...
    expect(within(guide).getByLabelText('Starter lyric preview')).toBeTruthy()
    expect(within(routeSummary).getByRole('button', { name: '첫 단계 샘플 듣기' })).toBeTruthy()
    expect(within(routeSummary).getByRole('button', { name: '둘째 단계 가사 적용' })).toBeTruthy()
    ...
    expect(within(guide).getByLabelText('Starter next action').textContent).toContain('샘플 준비 중')
    ...
    expect(within(utilities).getByText('도히도히 다이스키로 시작해요')).toBeTruthy()
    ...
    expect(handoffChecklist.textContent).toContain('WAV · lyrics.txt · notes.csv')
