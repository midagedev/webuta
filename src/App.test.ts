...
describe('App editing workflow', () => {
  ...
  it('surfaces a first-run guide with direct starter actions', async () => {
    ...
    const successMission = within(guide).getByLabelText('Beginner success mission')
    expect(successMission.textContent).toContain('1분 미션')
    expect(successMission.textContent).toContain('한글 한 줄을 보컬 WAV로 만들기')
    expect(successMission.textContent).toContain('샘플을 듣고, 한 줄 가사를 바꾼 뒤 First-Vocal-Sketch.wav로 받으면 첫 곡이 완성돼요.')
    expect(successMission.textContent).toContain('First-Vocal-Sketch.wav')
    expect(within(successMission).getByLabelText('Beginner success checkpoints')).toBeTruthy()
    expect(within(successMission).getByRole('button', { name: '미션 샘플 듣기' })).toBeTruthy()
    expect(within(successMission).getByRole('button', { name: '미션 가사 바꾸기' })).toBeTruthy()
    expect(within(successMission).getByRole('button', { name: '미션 WAV 받기' })).toBeTruthy()
    ...
    expect(sampleGallery.textContent).toContain('보컬로이드풍 훅 12개')
    ...
    expect(sampleGallery.textContent).toContain('Candy Crash')
    expect(sampleGallery.textContent).toContain('Velvet Orbit')
    expect(sampleGallery.textContent).toContain('장르, 템포, 가사 발음이 다른 샘플')
    const sampleDiversity = within(sampleGallery).getByLabelText('Starter sample diversity summary')
    expect(sampleDiversity.textContent).toContain('12곡')
    expect(sampleDiversity.textContent).toContain('12장르')
    expect(sampleDiversity.textContent).toContain('4밴드')
    expect(sampleDiversity.textContent).toContain('느림 · 미드 · 업 · 하이퍼')
    expect(sampleDiversity.textContent).toContain('받침 11곡')
    expect(sampleDiversity.textContent).toContain('C4-B5')
    expect(sampleDiversity.textContent).toContain('코드 12개')
    ...
    expect(sampleGallery.textContent).toContain('깨지는 전자 질감')
    expect(sampleGallery.textContent).toContain('미끄러지는 R&B')
    ...
    expect(neonMetrics.textContent).toContain('11 notes')
    expect(neonMetrics.textContent).toContain('받침 1')
    expect(neonMetrics.textContent).toContain('끝 2.0박')
    const rainMetrics = within(sampleGallery).getByLabelText('Rain Verse sample metrics')
    expect(rainMetrics.textContent).toContain('82 BPM')
    expect(rainMetrics.textContent).toContain('C4-A4')
    expect(rainMetrics.textContent).toContain('10 notes')
    expect(rainMetrics.textContent).toContain('받침 3')
    ...
    const starterRunway = within(handoffChecklist).getByLabelText('Starter reviewer runway')
    expect(starterRunway.textContent).toContain('Reviewer Runway')
    expect(starterRunway.textContent).toContain('2 JSON 파일로 공개 전 체크 완료')
    expect(starterRunway.textContent).toContain('01')
    expect(starterRunway.textContent).toContain('Listen')
    expect(starterRunway.textContent).toContain('청취 JSON')
    expect(starterRunway.textContent).toContain('02')
    expect(starterRunway.textContent).toContain('Handoff')
    expect(starterRunway.textContent).toContain('DAW JSON')
    expect(starterRunway.textContent).toContain('03')
    expect(starterRunway.textContent).toContain('Preflight')
    expect(starterRunway.textContent).toContain('No upload')
    expect(starterRunway.textContent).toContain('04')
    expect(starterRunway.textContent).toContain('Status')
    expect(starterRunway.textContent).toContain('터미널 확인')
    expect(starterRunway.textContent).toContain('05')
    expect(starterRunway.textContent).toContain('Accept')
    expect(starterRunway.textContent).toContain('최종 수락')
    expect(starterRunway.querySelector('a[href="/review/v3/index.html"]')).toBeTruthy()
    expect(starterRunway.querySelector('a[href="/review/wav-daw/index.html"]')).toBeTruthy()
    expect(starterRunway.querySelector('a[href="/review/index.html#evidence-preflight"]')).toBeTruthy()
