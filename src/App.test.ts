...
describe('App editing workflow', () => {
  ...
  it('surfaces a first-run guide with direct starter actions', async () => {
    ...
    expect(within(guide).getByText('듣기 → 가사 → 멜로디 → WAV')).toBeTruthy()
    expect(within(guide).getByText('처음 1분')).toBeTruthy()
    expect(within(guide).getByText('샘플 듣기 → 가사 적용 → 멜로디 만들기 → WAV 받기')).toBeTruthy()
    ...
    expect(within(path).getByText('가사 적용')).toBeTruthy()
    expect(within(path).getByText('3')).toBeTruthy()
    expect(within(path).getByText('멜로디 만들기')).toBeTruthy()
    ...
    expect(within(guide).getByRole('button', { name: '초보자 가사 적용' })).toBeTruthy()
    expect(within(guide).getByRole('button', { name: '초보자 멜로디 만들기' })).toBeTruthy()
    ...
    fireEvent.click(within(guide).getByRole('button', { name: '초보자 멜로디 만들기' }))
