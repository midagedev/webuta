...
describe('App editing workflow', () => {
  ...
  it('surfaces a first-run guide with direct starter actions', async () => {
    ...
    expect(beginnerStart.textContent).toContain('기본 샘플로 시작')
    expect(beginnerStart.textContent).toContain('샘플을 먼저 듣고 가사만 바꿔보세요.')
  ...
  it('makes a restored draft obvious and offers a one-tap return to the default sample', async () => {
    ...
    expect(beginnerStart.textContent).toContain('저장된 작업에서 시작')
    expect(beginnerStart.textContent).toContain('이어서 듣거나, 기본 샘플로 바로 돌아갈 수 있어요.')
