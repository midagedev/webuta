...
describe('UTAU sample renderer', () => {
  ...
  it('uses per-note vibrato depth when advancing the UTAU sample', async () => {
    ...
    const renderer = createUtauSampleRenderer(voicebank, audioContext)
    const withoutVibrato = await renderer.render({
      ...baseProject,
      notes: [
        {
          ...baseProject.notes[0],
    ...
    const withVibrato = await renderer.render({
      ...baseProject,
      notes: [
        {
          ...baseProject.notes[0],
          lyric: '라',
          duration: TICKS_PER_BEAT * 3,
          vibrato: { enabled: true, depthCents: 64, rateHz: 7.2, startPercent: 10 },
        },
      ],
    })
    const difference = withVibrato.samples.reduce(
      (sum, sample, index) => sum + Math.abs(sample - (withoutVibrato.samples[index] ?? 0)),
      0,
    )
