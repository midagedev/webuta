...
describe('LeftRail release readiness', () => {
  it('shows bundled V3 automated checks while keeping human listening review explicit', () => {
    ...
    expect(manualEvidence.textContent).toContain('공개 전 마지막 2개 파일')
    ...
    expect(manualEvidence.textContent).toContain('Evidence Preflight')
...
function makeProps(overrides: Partial<Record<string, unknown>> = {}) {
  ...
  const base = {
    ...
    notice: `${BUNDLED_UTAU_VOICEBANK_NAME}: 1892 aliases`,
