  ...
  try {
    ...
    const manifest = {
      ...
      nextSteps: [
        ...
        'Download handoff-report.local.json from the WAV/DAW handoff page, then run npm run release:evidence-status before npm run release:accept-evidence.',
...
<body>
  <main>
    ...
    <p>After downloading this JSON, download <code>handoff-report.local.json</code> from the DAW handoff page too, keep both files in Downloads, then use the review hub <code>Evidence Preflight</code> checker to validate both JSON files locally with no upload. Run <code>npm run release:evidence-status</code> before accepting both with <code>npm run release:accept-evidence</code>. Use explicit <code>--scores</code> and <code>--handoff</code> paths only when the files are somewhere else.</p>
  ...
  <script>
    ...
    function updateOutput() {
      ...
      problemList.innerHTML = problems.length === 0
        ? '<li>Ready: download this JSON, keep it beside the DAW handoff JSON in Downloads, check both files in Evidence Preflight, run npm run release:evidence-status, then run npm run release:accept-evidence.</li>'
...
function renderReadme({ indexHtmlPath, listeningTemplatePath, phrases, comparisons = [] }) {
  return [
    ...
    'After downloading this file, download `handoff-report.local.json` from the DAW handoff page too, keep both files in Downloads, then use the review hub `Evidence Preflight` checker to validate both JSON files locally with no upload. Run `npm run release:evidence-status` before accepting both with `npm run release:accept-evidence` and the final release audit. Use explicit `--scores` and `--handoff` paths only when the files are somewhere else.',
