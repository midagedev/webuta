...
async function waitForAppReady(page) {
  ...
  await page.getByLabel('Beginner success mission').getByText('1분 미션').waitFor({ timeout: DEFAULT_TIMEOUT_MS })
  await page.getByLabel('Beginner success mission').getByText('한글 한 줄을 보컬 WAV로 만들기').waitFor({ timeout: DEFAULT_TIMEOUT_MS })
  await page.getByLabel('Beginner success mission').getByRole('button', { name: '미션 샘플 듣기' }).waitFor({ timeout: DEFAULT_TIMEOUT_MS })
  await page.getByLabel('Beginner start panel').getByText('처음이면 여기부터').waitFor({ timeout: DEFAULT_TIMEOUT_MS })
  await page.getByLabel('Beginner start panel').getByRole('button', { name: '초보자 첫 버튼' }).waitFor({ timeout: DEFAULT_TIMEOUT_MS })
  await page.getByLabel('Beginner start panel').getByLabel('첫 사용 순서').getByText('1 듣기').waitFor({ timeout: DEFAULT_TIMEOUT_MS })
  await page.getByLabel('Beginner start panel').getByLabel('Top starter lyric editor').getByLabel('빠른 가사 입력').waitFor({ timeout: DEFAULT_TIMEOUT_MS })
  await page.getByLabel('Beginner start panel').getByLabel('Top starter lyric editor').getByRole('button', { name: '빠른 가사 적용' }).waitFor({ timeout: DEFAULT_TIMEOUT_MS })
  ...
  await starterSampleGallery.getByLabel('Starter sample selection guide').getByText('처음 듣는 기본 후렴').waitFor({ timeout: DEFAULT_TIMEOUT_MS })
  await starterSampleGallery.getByLabel('Starter sample selection guide').getByText('밝은 상승 멜로디').waitFor({ timeout: DEFAULT_TIMEOUT_MS })
  await starterSampleGallery.getByLabel('Starter sample selection guide').getByText('긴 끝음').waitFor({ timeout: DEFAULT_TIMEOUT_MS })
  await starterSampleGallery.getByLabel('Neon Lift sample metrics').getByText('128 BPM').waitFor({ timeout: DEFAULT_TIMEOUT_MS })
  await starterSampleGallery.getByLabel('Neon Lift sample metrics').getByText('A4-E5').waitFor({ timeout: DEFAULT_TIMEOUT_MS })
  ...
  await starterRoute.getByRole('button', { name: '둘째 단계 가사 바꾸기' }).waitFor({ timeout: DEFAULT_TIMEOUT_MS })
  await starterRoute.getByRole('button', { name: '셋째 단계 WAV 받기' }).waitFor({ timeout: DEFAULT_TIMEOUT_MS })
  const beginnerLaunchPad = page.locator('details[aria-label="Beginner launch pad"]')
  await beginnerLaunchPad.getByText('가사 자세히').waitFor({ timeout: DEFAULT_TIMEOUT_MS })
  await beginnerLaunchPad.evaluate((element) => {
    element.setAttribute('open', '')
  })
  await page.locator('details[aria-label="Beginner launch pad"][open]').waitFor({ timeout: DEFAULT_TIMEOUT_MS })
  ...
  await page.getByLabel('Starter handoff checklist').getByText('고급 도구').waitFor({ timeout: DEFAULT_TIMEOUT_MS })
