...
async function waitForAppReady(page) {
  ...
  await page.getByLabel('First run guide').getByText('START HERE').waitFor({ timeout: DEFAULT_TIMEOUT_MS })
  const starterRoute = page.getByLabel('First run route')
  await starterRoute.getByText('처음 3분').waitFor({ timeout: DEFAULT_TIMEOUT_MS })
  await starterRoute.getByLabel('Starter route summary').getByText('듣기', { exact: true }).waitFor({ timeout: DEFAULT_TIMEOUT_MS })
  await starterRoute.getByLabel('Starter route summary').getByText('가사', { exact: true }).waitFor({ timeout: DEFAULT_TIMEOUT_MS })
  await starterRoute.getByLabel('Starter route summary').getByText('저장', { exact: true }).waitFor({ timeout: DEFAULT_TIMEOUT_MS })
  ...
  await page.getByLabel('Starter lyric preview').getByText('도히도히 다이스키로 시작해요').waitFor({ timeout: DEFAULT_TIMEOUT_MS })
