...
async function waitForAppReady(page) {
  ...
  await page.getByLabel('Lyric input helper').getByText('한글 그대로 입력').waitFor({ timeout: DEFAULT_TIMEOUT_MS })
  ...
  await starterUtilities.getByText('기본 샘플').first().waitFor({ timeout: DEFAULT_TIMEOUT_MS })
  await starterUtilities.getByText('도 히 도 히 다 이 스 키').waitFor({ timeout: DEFAULT_TIMEOUT_MS })
