...
async function waitForAppReady(page) {
  ...
  await page.getByLabel('Starter lyric preview').getByText('현재 가사').waitFor({ timeout: DEFAULT_TIMEOUT_MS })
  ...
  await starterUtilities.getByText('작업 시작').waitFor({ timeout: DEFAULT_TIMEOUT_MS })
  await starterUtilities.getByText('필요한 것만 바로 꺼내기').waitFor({ timeout: DEFAULT_TIMEOUT_MS })
