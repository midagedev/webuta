...
export const STARTER_SAMPLES = [
  ...
  },
  {
    ...
    vocalFocus: '저음 슬라이드',
    bpm: 100,
    projectName: 'Velvet Orbit Vocal',
...
export function summarizeStarterSampleRenders(input = {}) {
  ...
  return {
    ...
    diversity: {
      ...
      bestForCount: new Set(renderedSamples.map((sample) => sample.bestFor)).size,
      listeningCueCount: new Set(renderedSamples.map((sample) => sample.listeningCue)).size,
      vocalFocusCount: new Set(renderedSamples.map((sample) => sample.vocalFocus)).size,
    ...
    samples: renderedSamples.map((sample) => ({
      ...
      listeningCue: sample.listeningCue,
      vocalFocus: sample.vocalFocus,
      bpm: sample.bpm ?? null,
