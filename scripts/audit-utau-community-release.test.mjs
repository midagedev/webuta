...
function makeReadme() {
  return [
    ...
    'The first-run starter shows `처음 시작`, `듣기 · 가사 · WAV`, `처음이면 여기부터`, `초보자 첫 버튼`, `1분 완성 루트`, `C -> G -> Am -> F`, `현재 프로젝트`, `고급 도구`, `DAW 번들`, and `다운로드 패키지` for the ZIP handoff path.',
...
function makeWavDawQa() {
  return [
    ...
    'Confirm the first-run guide shows `처음 시작`, `듣기 · 가사 · WAV`, `처음이면 여기부터`, `초보자 첫 버튼`, `현재 프로젝트`, `1분 완성 루트`, `C -> G -> Am -> F`, `01 샘플 듣기`, `02 가사 적용`, `03 WAV 받기`, `한글 그대로 입력`, `스타터 가사 라인`, `현재 가사`, `샘플 듣기`, `멜로디 추천`, `DAW 번들`, `렌더 후 ZIP`, `새 프로젝트`, `기본 샘플`, and `고급 도구`.',
