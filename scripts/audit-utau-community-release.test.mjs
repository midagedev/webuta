...
function makeReadme() {
  return [
    ...
    'The first-run starter shows `처음 시작`, `듣기 · 가사 · WAV`, `처음이면 여기부터`, `초보자 첫 버튼`, `첫 사용 순서`, `지금 할 일`, `빠른 가사 입력`, `빠른 가사 적용`, `가사 자세히`, `한국어 UTAU 모드`, `첫 완성 루트`, `C -> G -> Am -> F`, `현재 프로젝트`, `추가 작업`, `고급 도구`, `DAW 번들`, and `다운로드 패키지` for the ZIP handoff path.',
...
function makeWavDawQa() {
  return [
    ...
    'Confirm the first-run guide shows `처음 시작`, `듣기 · 가사 · WAV`, `처음이면 여기부터`, `초보자 첫 버튼`, `첫 사용 순서`, `지금 할 일`, `빠른 가사 입력`, `빠른 가사 적용`, `가사 자세히`, `한국어 UTAU 모드`, `현재 프로젝트`, `첫 완성 루트`, `C -> G -> Am -> F`, `01 샘플 듣기`, `02 가사 적용`, `03 WAV 받기`, `한글 그대로 입력`, `스타터 가사 라인`, `현재 가사`, `샘플 듣기`, `추가 작업`, `멜로디 추천`, `DAW 번들`, `렌더 후 ZIP`, `새 프로젝트`, `기본 샘플`, and `고급 도구`.',
