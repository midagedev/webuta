...
async function assertDefaultV3DemoReady(page) {
  ...
  await starterGuide.getByText('듣기 → 가사 → 멜로디 → WAV').waitFor({ timeout: DEFAULT_TIMEOUT_MS })
  const starterPath = page.getByLabel('Starter path')
  await starterPath.getByText('처음 1분').waitFor({ timeout: DEFAULT_TIMEOUT_MS })
  await starterPath.getByText('샘플 듣기 → 가사 적용 → 멜로디 만들기 → WAV 받기').waitFor({ timeout: DEFAULT_TIMEOUT_MS })
  await page.getByLabel('Starter action route').waitFor({ timeout: DEFAULT_TIMEOUT_MS })
  ...
  await page.getByLabel('Starter project utilities').waitFor({ timeout: DEFAULT_TIMEOUT_MS })
