...
async function assertDefaultV3DemoReady(page) {
  ...
  await starterSampleGallery.getByText('보컬로이드풍 훅 12개').waitFor({ timeout: DEFAULT_TIMEOUT_MS })
  ...
  await starterSampleGallery.getByRole('button', { name: 'Moon Signal 샘플 열기' }).waitFor({ timeout: DEFAULT_TIMEOUT_MS })
  ...
  await starterSampleGallery.getByRole('button', { name: 'Candy Crash 샘플 열기' }).waitFor({ timeout: DEFAULT_TIMEOUT_MS })
  await starterSampleGallery.getByRole('button', { name: 'Velvet Orbit 샘플 열기' }).waitFor({ timeout: DEFAULT_TIMEOUT_MS })
  const starterSampleDiversity = starterSampleGallery.getByLabel('Starter sample diversity summary')
  await starterSampleDiversity.getByText('12곡').waitFor({ timeout: DEFAULT_TIMEOUT_MS })
  await starterSampleDiversity.getByText('12장르').waitFor({ timeout: DEFAULT_TIMEOUT_MS })
  ...
  await starterSampleDiversity.getByText('받침 11곡').waitFor({ timeout: DEFAULT_TIMEOUT_MS })
